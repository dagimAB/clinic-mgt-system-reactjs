...
const initialize = async (silent = false) => {
  try {
    if (!silent) console.log("➡️ Creating staff core table...");
    await dbhelper.query(createStaffTable);
    if (!silent) console.log("✅ Staff table initialized successfully.");

    if (!silent)
      console.log(
        "➡️ Initializing dependent doctors table from staff bootstrap...",
      );
    await DoctorsModel.init();
    if (!silent) console.log("✅ Dependent doctors table initialized.");

    if (!silent)
      console.log(
        "➡️ Initializing dependent patients table from staff bootstrap...",
      );
    await PatientsModel.init();
    if (!silent) console.log("✅ Dependent patients table initialized.");

    if (!silent)
      console.log(
        "➡️ Initializing dependent nurses table from staff bootstrap...",
      );
    await NursesModel.init();
    if (!silent) console.log("✅ Dependent nurses table initialized.");

    if (!silent)
      console.log(
        "➡️ Initializing dependent lab technologist table from staff bootstrap...",
      );
    ...
    if (!silent) console.log("➡️ Checking for default admin seed...");
    const adminExists = await findById("ADM-001");
    if (!adminExists) {
      if (!silent) console.log("Seeding default admin...");
...
const addStaff = async (staff) => {
  ...
  if (!staff.id) {
    ...
    const countResult = await dbhelper.query(countStaffByRoleQuery, [
      staff.role,
    ]);
  ...
  if (newStaff.role === "DOCTOR") {
    try {
      ...
      console.log(
        `✅ Synced DOCTOR ${newStaff.id} to independent doctors table.`,
      );
    } catch (syncErr) {
      console.error(
        `❌ Sync failed for DOCTOR ${newStaff.id}:`,
        syncErr.message,
      );
    }
    ...
    try {
      ...
      console.log(
        `✅ Synced NURSE ${newStaff.id} to independent nurses table.`,
      );
    } catch (syncErr) {
      console.error(
        `❌ Sync failed for NURSE ${newStaff.id}:`,
        syncErr.message,
      );
    }
    ...
    try {
      ...
      console.log(
        `✅ Synced LAB_TECH ${newStaff.id} to independent lab tech table.`,
      );
    } catch (syncErr) {
      console.error(
        `❌ Sync failed for LAB_TECH ${newStaff.id}:`,
        syncErr.message,
      );
    }
  }
  ...
  if (updatedStaff) {
    if (updatedStaff.role === "DOCTOR") {
      try {
        ...
        console.log(
          `✅ Synced update for DOCTOR ${id} to independent doctors table.`,
        );
      } catch (syncErr) {
        console.error(
          `❌ Update sync failed for DOCTOR ${id}:`,
          syncErr.message,
        );
      }
      ...
      try {
        ...
        console.log(
          `✅ Synced update for NURSE ${id} to independent nurses table.`,
        );
      } catch (syncErr) {
        console.error(
          `❌ Update sync failed for NURSE ${id}:`,
          syncErr.message,
        );
      }
    }
